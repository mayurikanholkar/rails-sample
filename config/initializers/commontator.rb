...
Commontator.configure do |config|
  ...
  # Default: ->(controller) { controller.current_member }
  config.current_user_proc = ->(controller) { controller.current_member }
  ...
  config.user_name_proc = ->(member) { I18n.t('commontator.anonymous') }
  ...
  config.user_link_proc = ->(member, app_routes) { '' }
  ...
  # }
  config.user_avatar_proc = ->(member, view) {
    view.commontator_gravatar_image_tag(user, 1, s: 60, d: 'mm')
  }
  ...
  config.user_email_proc = ->(member, mailer) { member.try(:email) || '' }
  ...
  config.thread_read_proc = ->(thread, member) { true }
  ...
  config.thread_moderator_proc = ->(thread, member) { false }
  ...
  #
  # Default: ->(current_member, query) {
  #   current_member.class.where('username LIKE ?', "#{query}%")
  # }
  config.user_mentions_proc = ->(current_member, thread, query) {
    current_member.class.where('username LIKE ?', "#{query}%")
  }

  config.user_name_proc = lambda { |member| I18n.t('commontator.anonymous') }
end
